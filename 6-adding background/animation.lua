...
function create_animation (w , h, x , y , s_x, steps,name,to_animate,fps,max_speed,max_angle,fallback_to)

  local anim= {} -- create an object
  ...
  anim.max_speed=max_speed
  anim.max_angle=max_angle
  anim.fallback_to=fallback_to
...
function load_animation (image,animation)
  for i = 1,animation.steps
  do
    animation.slices[i]= load_slice(image,i,animation)
  end
end
